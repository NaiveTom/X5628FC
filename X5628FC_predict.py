...
STEP_SIZE_TEST = 32 # test_generator.batch_size

test_generator = test_datagen.flow_from_directory(directory='test/', target_size=(20002,4),
                                                  color_mode='grayscale',
                                                  classes=['pos','neg'],
                                                  class_mode='categorical', # "categorical"会返回2D的one-hot编码标签,"binary"返回1D的二值标签."sparse"返回1D的整数标签
                                                  batch_size=STEP_SIZE_TEST,
                                                  shuffle = False) # 不要乱
...
# 先测算准确度
score = clf.evaluate_generator(generator=test_generator, steps=STEP_SIZE_TEST)
...
y_prob = clf.predict_generator(generator=test_generator, steps=STEP_SIZE_TEST)
